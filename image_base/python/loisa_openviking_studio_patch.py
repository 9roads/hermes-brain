...
BOOTSTRAP_SCRIPT = r'''<script id="loisa-openviking-studio-bootstrap">
(function () {
  ...
  var encoded = params.get("loisa_ov_bootstrap");
  if (!encoded) {
    return;
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error("Invalid Loisa OpenViking Studio bootstrap payload");
    }
...
def main() -> None:
    ...
    if "loisa-openviking-studio-bootstrap" in source:
        print(f"Loisa OpenViking Studio bootstrap already patched: {STUDIO_INDEX}")
        return
    ...
    print(f"Patched Loisa OpenViking Studio bootstrap: {STUDIO_INDEX}")
