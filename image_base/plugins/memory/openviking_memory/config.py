...
PUBLIC_VIKING_SCOPES = {"agent", "resources", "user"}
...
class ProviderConfig:
    ...
    def from_env(cls, **runtime: Any) -> "ProviderConfig":
        return cls(
            ...
            api_key=_first_nonempty(os.environ.get("OPENVIKING_API_KEY"), os.environ.get("OPENVIKING_ROOT_API_KEY")),
    ...
    def validate_public_uris(self, uris: Iterable[str], *, allow_root: bool = False) -> list[str]:
        allowed = []
        for uri in uris:
            cleaned = _clean(uri)
            if not cleaned:
            ...
            if cleaned == "viking://":
                if allow_root:
                    ...
                    continue
                raise ValueError("URI must use a public OpenViking scope")
            parsed = urlparse(cleaned)
            if parsed.scheme != "viking" or parsed.netloc not in PUBLIC_VIKING_SCOPES:
                scopes = ", ".join(sorted(PUBLIC_VIKING_SCOPES))
                raise ValueError(f"URI must use a public OpenViking scope: {scopes}")
    ...
    def validate_resource_uri(self, value: str) -> str:
        cleaned = self.validate_public_uris([value])[0]
