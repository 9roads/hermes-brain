    ...
    *,
    ...
    try:
        ...
        raise BootstrapRequestError(
            f"Phoenix backend returned HTTP {error.code}",
            status=error.code,
            data=data,
        ) from error
...
def parse_bootstrap_response(data: Any) -> BootstrapSessionResponse:
    ...
    if not isinstance(payload, dict):
        raise BootstrapRequestError("Phoenix backend returned a non-object response", data=data)
    ...
    if not composio_session_id:
        raise BootstrapRequestError("Phoenix backend response omitted composio_session_id", data=data)

    if not missing_tool_url_template:
        raise BootstrapRequestError(
            "Phoenix backend response omitted missing_tool_url_template",
            data=data,
        )

    if schemas.TOOLKIT_SLUG_PLACEHOLDER not in missing_tool_url_template:
        raise BootstrapRequestError(
            "Phoenix backend response missing-tool URL lacks {toolkit_slug}",
            data=data,
        )
...
def load_config(environ: dict[str, str] | None = None) -> PhoenixConfig:
    source = environ or os.environ
    backend_url = read_env(source, "PHOENIX_BACKEND_URL").rstrip("/")
    workspace_id = read_env(source, "PHOENIX_WORKSPACE_ID")
    token = read_env(source, "PHOENIX_HERMES_PLUGIN_TOKEN")
...
def build_url(config: PhoenixConfig, path: str) -> str:
    return (
