...
class LoisaComposioSessionTests(unittest.TestCase):
    def tearDown(self) -> None:
        for name in list(sys.modules):
            if name.startswith("loisa_composio_session_test_"):
    ...
    def test_backend_bootstrap_request_uses_auth_headers_and_expected_payload(self) -> None:
        ...
        response = plugin.client.create_tool_router_session(
            ...
            environ={
                "LOISA_BACKEND_URL": "https://backend.test/",
                "LOISA_WORKSPACE_ID": "workspace one",
                "LOISA_HERMES_PLUGIN_TOKEN": "plugin-token",
        ...
        self.assertEqual(request.get_header("User-agent"), "loisa-hermes-composio-session/0.1.0")
    ...
    def test_profile_removes_static_mcp_and_documents_runtime_requirements(self) -> None:
        ...
        self.assertIn("- loisa-composio-session", config)
        ...
        self.assertIn('shutil.which("kernel")', healthcheck)
        ...
        self.assertIn("LOISA_BACKEND_URL", healthcheck)
        self.assertIn("LOISA_HERMES_PLUGIN_TOKEN", healthcheck)
...
def load_plugin_package():
    module_name = f"loisa_composio_session_test_{uuid.uuid4().hex}"
