...
TOOLSET = "loisa_trigger_subscriptions"
...
def create_trigger(args: dict[str, Any], **_: Any) -> str:
        ...
        return json_tool_response(
            {
                ...
                "message": "Loisa did not return the Hermes route name needed for CLI subscribe.",
        ...
        return json_tool_response(
            {
                ...
                "message": "Loisa did not return the route secret needed for CLI subscribe.",
                "backend": created.data,
            }
        )
    ...
    if finalize and not finalize.ok:
        return json_tool_response(
            {
                "ok": False,
                "error": "loisa_trigger_finalize_failed",
                "trigger": created.data,
                "hermes_route": {
                    "name": route_name,
        ...
        return json_tool_response(
            {
                "ok": False,
                "error": "backend_response_missing_route_name",
                "message": "Loisa deleted the trigger but did not return a Hermes route to remove.",
...
def validate_create_args(args: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if webhook is not None and not isinstance(webhook, dict):
        return {
            ...
            "message": "webhook must be an object with Loisa-supported Hermes route options.",
        }
...
def validate_webhook_delivery(webhook: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if deliver not in SUPPORTED_DELIVERY_TARGETS:
        return {
            ...
            "message": "Loisa trigger subscriptions support only log and slack delivery.",
    ...
    try:
        ...
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "loisa-hermes-trigger-subscriptions/0.1.0",
            },
        )
        ...
        data.setdefault("error", f"loisa_backend_http_{error.code}")
        ...
        return BackendResponse(
            ...
            {
                "ok": False,
                "error": "loisa_backend_request_failed",
                "message": str(error),
            },
        )
...
def load_config() -> LoisaConfig:
    backend_url = read_env("LOISA_BACKEND_URL")
    workspace_id = read_env("LOISA_WORKSPACE_ID")
    token = read_env("LOISA_HERMES_PLUGIN_TOKEN")
    return LoisaConfig(
...
def profile_name() -> str:
    return os.getenv("LOISA_HERMES_PROFILE_NAME", "loisa").strip() or "loisa"


def build_url(
    config: LoisaConfig, path: str, *, query: dict[str, Any] | None = None
) -> str:
    base = (
...
def redact(value: Any, secrets: list[str] | None = None) -> str:
    ...
    token = os.getenv("LOISA_HERMES_PLUGIN_TOKEN", "").strip()
    if token:
