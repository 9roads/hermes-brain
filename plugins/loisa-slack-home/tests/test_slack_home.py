...
class SlackHomeTests(unittest.TestCase):
    ...
    def test_scores_explicit_loisa_channel_above_general(self) -> None:
        loisa = self.slack_home.SlackChannelCandidate(
            channel_id="C_LOISA",
            name="loisa-updates",
            is_member=True,
            num_members=10,
        ...
        selected = sorted([general, loisa], key=self.slack_home.score_candidate, reverse=True)[0]

        self.assertEqual(selected.channel_id, "C_LOISA")

    def test_apply_home_channel_sets_process_env(self) -> None:
        writes = install_hermes_config_stub()

        result = self.slack_home.apply_home_channel("C123", name="loisa", gateway=None)
    ...
    def test_sync_gateway_home_channel_updates_runtime_config(self) -> None:
        ...
        self.assertEqual(gateway.config.platforms[platform].home_channel.name, "loisa")
...
def load_plugin_module():
    module_name = f"loisa_slack_home_test_{uuid.uuid4().hex}"
